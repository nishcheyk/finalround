...
export class AppointmentService {
  ...
  static async create(
    ...
    /* The above code is creating a new Date object based on the `startTime` of an appointment. It then
   sets the hours of this date object to be 24 hours before the original time. Finally, it
   calculates the delay in milliseconds between the reminder date and the current date and time.
   This delay can be used to schedule a reminder or notification for the appointment. */
  ...
  static async rescheduleAppointment(
    ...
    if (
      serviceId &&
      serviceId !== (appointment.service as any)._id.toString()
    ) {
      const service = await Service.findById(serviceId);
      if (!service) throw createHttpError(404, "Service not found");
